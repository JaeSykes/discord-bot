...
class ReminderView(View):
    def __init__(self, item_name):
        super().__init__(timeout=3600)  # 1h na reakci, pak se view "odpojí"
        self.item_name = item_name
    ...
    @discord.ui.button(label="Stále mám", style=discord.ButtonStyle.secondary)
    async def still_have_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Jen potvrdí, bez změny stavu, ale můžeme resetnout reminder_stage,
        # nebo jen poděkovat – tady jen poděkujeme
...
async def on_ready():
    ...
    # Persistentní view pro ReminderView a ItemLoanView
    # (aby tlačítka fungovala i po restartu bota)
    for item_name, _ in ITEMS_LIST:
        bot.add_view(ItemLoanView(item_name))

    # ReminderView je DM-only, není nutné ho registrovat globálně,
    # ale pokud chceš 100% persistenci i po restartu, můžeš:
    # bot.add_view(ReminderView("dummy"))
